...
urls <- "https://www.usitc.gov/data/gravity/itpd_s/itpd_s_r1.1.zip"
...
while (continue_reading) {
  ...
  chunk[, flag_itpds := as.integer(flag_itpds)]

  dbWriteTable(con, "itpds_tmp", chunk, append = TRUE)
...
yrs <- as.data.table(dbGetQuery(con, "select distinct year from itpds_tmp"))
yrs <- sort(as.integer(yrs$year))

dbExecute(con, "create index idx_year ON itpds_tmp (year)")
...
lapply(
  ...
  function(y) {
    ...
    if (any("itpd_sectors" %in% tbls)) {
      itpd_sectors <- as.data.table(dbGetQuery(con, "select distinct broad_sector from itpds_tmp"))
    ...
    if (!any("itpd_industries" %in% tbls)) {
      itpd_industries <- as.data.table(dbGetQuery(con, "select distinct industry_descr, industry_id from itpds_tmp"))
    ...
    chunk <- as.data.table(dbGetQuery(con,
      sprintf("select * from itpds_tmp where year = %s", y)))
    ...
    setnames(chunk, c("exporter_dynamic_code", "importer_dynamic_code"),
      c("exporter_iso3_dynamic", "importer_iso3_dynamic"))

    cls <- c("year", "importer_iso3", "importer_iso3_dynamic",  "exporter_iso3", "exporter_iso3_dynamic", 
            "broad_sector_id", "industry_id",  "flag_mirror", "flag_zero", "flag_itpds", "trade")

    setcolorder(chunk, cls)

    setorder(chunk, year, importer_iso3, exporter_iso3, broad_sector_id, industry_id)

    dbWriteTable(con, "itpds", chunk, append = TRUE)
...
dbSendQuery(con, "drop table itpds_tmp")
